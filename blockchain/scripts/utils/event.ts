...
async function transferEvent() {
  ...
  let abi = require("../../artifacts/contracts/Token.sol/Token.json").abi;
  let iface = new ethersType.utils.Interface(abi);

  logs.forEach(async (logs) => {
    //실제로 이벤트 로그 내용을 분석하기 위해서는 각각의 트랜잭션 receipt를 가져와서 처리해야 한다.
    const receipt: any = await ethers.provider.getTransactionReceipt(
