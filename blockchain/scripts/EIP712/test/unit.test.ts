    ...
    it("step 03) get signature", async () => {
      ...
      const msgParams = {
        domain: {
          ...
          chainId: ethersType.BigNumber.from(chainId),
      ...
      const verifiedAddress = ethersType.utils.verifyTypedData(
        msgParams.domain,
        msgParams.types,
        msgParams.message,
        signature
      ...
      const recoverSig = await exampleEIP712.validateSigner(
        structHash,
        signature
      );
